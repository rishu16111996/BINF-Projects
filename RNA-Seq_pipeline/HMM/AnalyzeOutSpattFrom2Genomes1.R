#!/usr/bin/env Rscript
# AnalyzeOutSpattFrom2Genomes1.R
options("scipen" = 30)
...
spattMetrics$underPresented <- 0 

for (i in 1:nrow(spattMetrics)){
    if (spattMetrics$occurrence1[i] < spattMetrics$occurrence2[i]){
        spattMetrics$underPresented[i] <- "Yes"
    }
    else{
        spattMetrics$underPresented[i] <- "No"
    }
}
...
write.table(spattMetrics, file="out1.txt", sep="\t", row.names = FALSE)
